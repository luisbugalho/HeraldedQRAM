...
def make_tree(layer):
    tree = {i:{"child_left":None,"child_right":None,"parent":None} for i in range(1,2**layer+1)}
...
p_value = round(float(sys.argv[2]),2) # 0 -> deterministic || 1 -> probabilistic
p_efficiency = int(sys.argv[3])


trees = {}
trees[0] = {}
trees[0][1] = {"child_left":None,"child_right":None,"parent":None}
        ...
        if self.layer == 1:
            ...
            while True:
                ...
                if self.state == 2:
                    ...
                    # print("Creating electron " + str(self.qubit_e_right) + " at " + str(dict_eletron[self.qubit_e_right]))
                ...
                if self.state == 3:
                    ...
                    #print(self.name_right + ": receiving reset signal from rcv protocol " )
                    if self.node.subcomponents["QuantumMemory" + self.name_right].busy:
                        yield self.await_program(self.node.subcomponents["QuantumMemory" + self.name_right])
                    self.reset_cavity_electron(flag_delete=True)
                    self.state = 1
        ...
        if int(self.index) > 1 : # does not receive a photon in cavity protocol, only entangling protocol
            while True:
                ...
                if self.state == 1:
                    ...
                    # print("Creating electron " + str(self.qubit_e_left) + " at " + str(dict_eletron[self.qubit_e_left]))

                    self.send_signal("emit" + str(self.index - 1))
                    #print(self.name_left + ": sending emit signal to emit protocol" )

                    #print(self.name_left + ": await photon reception ")
                    yield self.await_port_input(self.node.ports["port_" + str(self.index) + str(self.index - 1) + "layer_" + str(self.layer)])
                    #print(self.name_left + ": input queue" + str(self.node.ports["port_" + str(self.index) + str(self.index - 1) + "layer_" + str(self.layer)].input_queue))
                    photon = self.node.ports["port_" + str(self.index) + str(self.index - 1) + "layer_" + str(self.layer)].rx_input().items[0]
                    #print(self.name_left + ": receiving photon " + str(photon))
                    ...
                    if m == 0:
                        ...
                        if self.node.subcomponents["QuantumMemory" + self.name_left].busy:
                            yield self.await_program(self.node.subcomponents["QuantumMemory" + self.name_left])
                        # print("electron after receiving is at :" + str(self.node.subcomponents["QuantumMemory" + self.name_left].peek(1)[0].qstate.dm))
                        self.send_signal("success" + str(self.index))
...
class LinkProtocol(NodeProtocol):    
    def __init__(self, node, index, layer):
        ...
        self.flag_deterministic = False
        p_aux = rd.random()
        if p_aux > p_value :
            self.flag_deterministic = True
        #print(str(self.layer) + ' looking at index ' + str(self.index))
        ...
        if self.layer == 1:
            ...
            while True:
                ...
                if self.state == 1:
                    ...
                    #print(self.name + ": linking receiving sucess from rcv protocol")
                    self.success = 1
                    self.send_signal("success" + str(self.index))
                    yield self.await_signal(circuit_protocol.subprotocols["link" + str(self.index+1) + 'layer_' + str(self.layer)],"reset" + str(self.index+1))
            ...
            while True:
                if self.state == 0:
                    ...
                    if self.index % 2 == 0: #at the first linking layer you listen for entanglement success from receive protocol
                        ...
                        yield link_expr
                        #print(self.name + ": receiving ok for linking on " + str(link_expr.first_term.value) + " | " + str(link_expr.second_term.value))
                        ...
                        link_expr_right = self.await_signal(circuit_protocol.subprotocols["link" + str(self.right_child) + 'layer_' + str(self.layer)],"success" + str(self.right_child))
                        link_expr = link_expr_left | link_expr_right
                        yield link_expr
                        #print(self.name + ": receiving ok for linking on " + str(link_expr.first_term.value) + " | " + str(link_expr.second_term.value))
                    ...
                    if self.left_ready and self.right_ready:
                        #print(str(self.name) + ": ready to link")
                        self.state = 1
                ...
                if self.state == 1:
                    ...
                    if not self.flag_deterministic :
                        ...
                        if m == 0:
                            ...
                            self.left_ready = False
                            self.right_ready = False
                            #print(self.name + ": linking FAILED")
                            self.send_signal("reset"+ str(self.index))
                        else:
                            if self.node.subcomponents["QuantumMemory" + self.name_left].busy:
                                yield self.await_program(self.node.subcomponents["QuantumMemory" + self.name_left])
    ...
    def link2d(self,electron):
        ...
        noisedict_CNOT_n[self.qubit_e_right] = 1
            ...
            if expr_se.second_term.value:
                ...
                if entanglement_success == largest_layer:
                    ...
                    if self.counter == num_round:
                        ...
                        print(ns.sim_time())
                        pathname = "All_Data_Raw_New/P1_" + str(p_value) +  "_P2_" + str(p_efficiency)
