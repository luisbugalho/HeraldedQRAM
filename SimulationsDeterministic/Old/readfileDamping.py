...
PLOT_GUIDES = [(T1[1],T2[0],CNOT_ERROR[0]),(T1[2],T2[0],CNOT_ERROR[0]),(T1[0],T2[0],CNOT_ERROR[0])]
...
for i in range(0,len(data_tree)):
	print(data_tree[i])
	plt.errorbar(data_points,data_tree[i],data_tree_std[i],label='Binary Tree Fidelity - T1 = ' + str("{:.0e}".format(PLOT_GUIDES[i][0])))
...
#plt.yscale('log')
plt.title('Fidelity Scaling -  Damping Noise')
plt.xscale('log')
#plt.yscale('log')
plt.ylabel('Fidelity')
#plt.ylim([0.98, 1.001])
