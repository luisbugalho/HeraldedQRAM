...
def f1(t1,t2):
	return 1 - ft(t1) - ft(t2) + ft(t1)*ft(t2)

def f2(t1,t2,tt1,tt2,p1):
	return (1 + f1(t1,t2)*(1-p1)**2*(1-ft(tt1))*(1-ft(tt2)) + ft(tt1)*ft(tt2))/2
...
def f3n(pairen,p1):
	...
	tt1= pairen[1][0]
	tt2= pairen[1][1]
	return (1-f2(t1,t2,tt1,tt2,p1))/f2(t1,t2,tt1,tt2,p1)
...
layers = 12
T1 = [ 2*10**7, 2*10**8 , math.inf]
T2 = [ 10**7, 10**8 , math.inf]
CNOT_ERRORS = [ 0.01, 0.001, 0.0001, 0.00001 , 0]
cnot_fidelity =  [1-err for err in CNOT_ERRORS]
CNOT_ERROR = [4 / 3 * (1 - err ** 2) for err in cnot_fidelity]


fig, (ax1, ax2) = plt.subplots(2, sharex=True)
fig.suptitle('Fidelity Scaling -  Dephasing + Damping Noise + Noisy CNOTs')
...
PLOT_GUIDES = [(T1[0],T2[0],CNOT_ERROR[0]),(T1[0],T2[0],CNOT_ERROR[1]),(T1[0],T2[0],CNOT_ERROR[2]),(T1[0],T2[0],CNOT_ERROR[3]),(T1[0],T2[0],CNOT_ERROR[4])]


for plot_guides in PLOT_GUIDES:
	data_array = []
	for i in range(2,layers+1):
		data_unstructured = np.load('All_Data_New/qRAM_teleportation_fidelities_'+str(i) + '.npy',allow_pickle=True)
		...
		for k in range(0,len(data_unstructured)):
			...
			treedict_a = {(j,k): [[0.,0.],[0.,0.]] for j in range(1, i + 1) for k in range(0, 2 ** (int(j - 1)) )} # every pair used indexed by the starting qubit, hence 2**layer -1
			...
			for qubit in dict_eletron:
				...
				if dict_eletron[qubit][1] == 1 and dict_eletron[qubit][0] == 0:
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0])][0][0] += noisedict_eletron[qubit][0]/t1
					#print(treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0])])

				elif dict_eletron[qubit][1] == 1 and dict_eletron[qubit][0] == 1:
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0]-1)][0][1] += noisedict_eletron[qubit][0]/t1

				elif dict_eletron[qubit][0] == 0:
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0])][0][0] += noisedict_eletron[qubit][0]/t1

				elif dict_eletron[qubit][0] % 2 == 1 and dict_eletron[qubit][3] == 'right': 											# EVEN QUBITS FIRST STEP
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0])][0][0] += noisedict_eletron[qubit][0]/t1

				elif dict_eletron[qubit][0] % 2 == 0 and dict_eletron[qubit][3] == 'right': 											# ODD QUBITS FIRST STEP
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0]-1)][0][1] += noisedict_eletron[qubit][0]/t1

				elif dict_eletron[qubit][0] % 2 == 0 and dict_eletron[qubit][3] == 'left': 												# EVEN QUBITS SECOND STEP
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0])][0][0] += noisedict_eletron[qubit][0]/t1

				elif dict_eletron[qubit][0] % 2 == 1 and dict_eletron[qubit][3] == 'left': 												# ODD QUBITS SECOND STEP
					treedict_a[(dict_eletron[qubit][1],dict_eletron[qubit][0]-1)][0][1] += noisedict_eletron[qubit][0]/t1

			
			for qubit in dict_nuclear:
				if dict_nuclear[qubit][2] == 1:
					treedict_b[(dict_nuclear[qubit][1],dict_nuclear[qubit][0])] += noisedict_nuclear[qubit][1]/t1/100
					continue

				if dict_nuclear[qubit][1] == 1 and dict_nuclear[qubit][0] == 0:
					treedict_a[(dict_nuclear[qubit][1],dict_nuclear[qubit][0])][1][0] += noisedict_nuclear[qubit][0]/t1/100

				elif dict_nuclear[qubit][1] == 1 and dict_nuclear[qubit][0] == 1:
					treedict_a[(dict_nuclear[qubit][1],dict_nuclear[qubit][0]-1)][1][1] += noisedict_nuclear[qubit][0]/t1/100

				elif dict_nuclear[qubit][0] == 0:
					treedict_a[(dict_nuclear[qubit][1],dict_nuclear[qubit][0])][1][0] += noisedict_nuclear[qubit][0]/t1/100

				elif dict_nuclear[qubit][0] % 2 == 1 and dict_nuclear[qubit][3] == 'right': 											# EVEN QUBITS FIRST STEP AFTER NUCLEAR
					treedict_a[(dict_nuclear[qubit][1],dict_nuclear[qubit][0])][1][0] += noisedict_nuclear[qubit][0]/t1/100

				elif dict_nuclear[qubit][0] % 2 == 0 and dict_nuclear[qubit][3] == 'right': 											# ODD QUBITS FIRST STEP AFTER NUCLEAR
					treedict_a[(dict_nuclear[qubit][1],dict_nuclear[qubit][0]-1)][1][1] += noisedict_nuclear[qubit][0]/t1/100
			...
			for j in range(1, i+1):
				...
				for key in treedict_a:
					if key[0] == j and key[1] % 2 == 1:
						term1 = term1*f2n(treedict_a[key],cnot_error)
					elif key[0] == j and key[1] % 2 == 0:
						term1 = term1*f2n(treedict_a[key],0)
...
for i in range(0,len(data_tree)):
	print(data_tree[i])
	ax1.errorbar(data_points,data_tree[i],data_tree_std[i],label='Binary Tree Fidelity - CNOT Error = ' + str("{:.0e}".format(CNOT_ERRORS[i])))


ax1.set_title('Fidelity Scaling -  Dephasing + Damping Noise: T1 = 2 T2 = 2x10^7')
ax1.set_xscale('log')
#plt.yscale('log')
...
PLOT_GUIDES = [(T1[1],T2[1],CNOT_ERROR[0]),(T1[1],T2[1],CNOT_ERROR[1]),(T1[1],T2[1],CNOT_ERROR[2]),(T1[1],T2[1],CNOT_ERROR[3]),(T1[1],T2[1],CNOT_ERROR[4])]
...
for i in range(0,len(data_tree)):
	print(data_tree[i])
	ax2.errorbar(data_points,data_tree[i],data_tree_std[i],label='Binary Tree Fidelity - CNOT Error = ' + str("{:.0e}".format(CNOT_ERRORS[i])))


ax2.set_title('Fidelity Scaling -  Dephasing + Damping Noise: T1 = 2 T2 = 2x10^8')
ax2.set_xscale('log')
#plt.yscale('log')
