...
for i in range(0,len(data_tree)):
	print(data_tree[i])
	plt.errorbar(data_points,data_tree[i],data_tree_std[i],label='Binary Tree Fidelity - CNOT Error = ' + str("{:.0e}".format(CNOT_ERRORS[i])))
...
#plt.yscale('log')
plt.title('Fidelity Scaling -  CNOTs Noise')
plt.xscale('log')
#plt.yscale('log')
plt.ylabel('Fidelity')
